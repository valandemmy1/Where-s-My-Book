...
export async function onRequestPost({request,env}){
  // Accounts are optional. A failed session lookup must never block book registration.
  let user=null;
  try{ user=await getUser(request,env); }catch(err){ console.error('Optional auth lookup failed',err); user=null; }
