...
export async function onRequestPost({request,env}){
  ...
  for(let attempt=0;attempt<5;attempt++){
    const id=makeId();
    try{
      await env.DB.prepare(`INSERT INTO books (id,title,author,isbn,start_city,start_region,start_country,starter_note) VALUES (?,?,?,?,?,?,?,?)`).bind(id,book.title,book.author,book.isbn||null,book.start_city,book.start_region||null,book.start_country,book.starter_note||null).run();
      const created=await env.DB.prepare('SELECT * FROM books WHERE id=?').bind(id).first();
      return out({book:created},201);
