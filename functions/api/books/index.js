...
export async function onRequestPost({request,env}){
  const user=await getUser(request,env); if(!user)return out({error:'Please sign in before registering a book.'},401);
  ...
  for(let attempt=0;attempt<5;attempt++){
    const id=makeId();
    try{
      await env.DB.prepare(`INSERT INTO books (id,title,author,isbn,start_city,start_region,start_country,starter_note,owner_user_id) VALUES (?,?,?,?,?,?,?,?,?)`).bind(id,book.title,book.author,book.isbn||null,book.start_city,book.start_region||null,book.start_country,book.starter_note||null,user.id).run();
